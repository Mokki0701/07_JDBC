...
public class DepartmentView {
	...
	public void displayMenu() {
		...
		do {
			try {
				...
				System.out.println("6. 부서에 속한 명단 조회");
				...
				case 5 : changeDepartment(); break;
				case 6 : selectPerson(); break;
	...
	private void deleteDepartment() throws SQLException {
		...
		int check = dao.checkDepartment(title);
		if(check == 0) {
		...
		if(ch == 'Y') {
			int result = dao.deleteDepartment(title);
	...
	private void changeDepartment() throws SQLException {
		...
		System.out.print("변경될 부서명 : ");
		String inputTitle = sc.next();
		

		int result = dao.changeDepartment(code, title, inputTitle);
		if(result > 0) System.out.println("\n[변경 완료]\n");
		else 					 System.out.println("\n[변경 실패]\n");
	...
	private void selectPerson() throws SQLException {
		
System.out.println("\n----- 부서 직원 조회 ------\n");
		
		System.out.print("조회할 부서 코드 : ");
		String code = sc.next();

		System.out.print("조회할 부서명 : ");
		String title = sc.next();
		
		List<Person> deptList = dao.selectPerson(code, title);
		
		if(deptList.isEmpty()) System.out.println("해당 부서는 존재하지 않습니다.");
		else {
