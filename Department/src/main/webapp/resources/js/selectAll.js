...
for(let btn of deleteBtnList){
    ...
    btn.addEventListener("click", e =>{
        ...
        // 근데 이러면 엄청 복잡하거나 자료가 많은 곳에서는 중복으로 내가 찾던걸 못찾거나, 시간 요소가 많이 소요되지않을까?
        ...
        if(confirm(`${deptId} 부서를 정말 삭제 하시겠습니까?`)){
            ...
            form.method = "POST";
            console.log(form); // 근데 JS는 왜이렇게 하기가 싫지

            // input type = "hidden" 생성
            const input = document.createElement("input");
            input.type = "hidden";
            input.value = deptId; // 값으로 부서 코드 대입
            input.name = "deptId"; // 피라미터 키 값 지정
